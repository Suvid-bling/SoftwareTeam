...
PROJECT_DISPLAY_NAME = "SdeTeam"


def get_metagpt_package_root():
    """Get the root directory of the installed package."""
    package_root = Path(sdeteam.__file__).parent.parent
    logger.info(f"{PROJECT_DISPLAY_NAME} package root set to {str(package_root)}")
    return package_root
...
def get_metagpt_root():
    ...
    if project_root_env:
        ...
        logger.info(f"{PROJECT_DISPLAY_NAME} PROJECT_ROOT set from environment variable to {str(project_root)}")
    else:
...
SOURCE_ROOT = METAGPT_ROOT / "sdeteam"
PROMPT_PATH = SOURCE_ROOT / "prompts"
SKILL_DIRECTORY = SOURCE_ROOT / "skills"
TOOL_SCHEMA_PATH = METAGPT_ROOT / "sdeteam/tools/schemas"
TOOL_LIBS_PATH = METAGPT_ROOT / "sdeteam/tools/libs"
...
# Reporter
SDETEAM_REPORTER_DEFAULT_URL = os.environ.get("SDETEAM_REPORTER_URL", os.environ.get("METAGPT_REPORTER_URL", ""))
...
# SWE agent
SWE_SETUP_PATH = get_metagpt_package_root() / "sdeteam/tools/swe_agent_commands/setup_default.sh"
